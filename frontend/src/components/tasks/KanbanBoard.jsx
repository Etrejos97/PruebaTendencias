...
export default function KanbanBoard({ project, user, onBack }) {
  ...
  const moveTask = async (task, newStatus) => {
    try {
      ...
      setTasks((t) =>
        t.map((x) => (x.id === task.id ? { ...x, status: newStatus } : x))
      );
    } catch (e) {
    ...
    <div>
                          ...
                          {task.tags?.length > 0 && (
                            <div className="task-tags">
                              {task.tags.map((tag, i) => {
                                ...
                                const key =
                                  typeof tag === "string" ? tag : tag.id ?? i;
                                return (
        ...
        >
          <div className="modal">
            ...
            <form onSubmit={saveTask}>
              ...
              {/* ── Comentarios: solo visible al editar una tarea existente ── */}
              {taskModal.task && (
                <div
                  className="field"
                  style={{
          ...
          {/* ↑ cierra <div className="modal"> */}
        </div>
        // ↑ cierra <div className="overlay">
      )}
      {/* ↑ cierra {taskModal && ( */}
      ...
      {membersOpen && (
        <div
          className="overlay"
          onClick={(e) =>
            e.target === e.currentTarget && setMembersOpen(false)
          }
        >
          <div className="modal" style={{ maxWidth: "400px" }}>
